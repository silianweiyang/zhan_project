...
// "colorize" the widget name
    $.widget( "flowchart.flowchart", {
        ...
        options: {
            ...
            defaultLinkColor: '#70B9D2',
            defaultSelectedLinkColor: '#d28a71',
            linkWidth: 10,
            grid: 20,
            multipleLinksOnOutput: true,
            multipleLinksOnInput: true,
            linkVerticalDecal: 0,
            onOperatorDbSelect:function (operateorId) {   //新增双击事件
                var tempArry = operateorId.split("_");
                if(tempArry && tempArry.length >2){
                    var operatorType = tempArry[2];
                    if(operatorType == "process"){
                        $("#composeApiMgr_bianPai_descApiButton").attr("operateorId",operateorId);
                        $("#composeApiMgr_bianPai_descApiButton").trigger("click");
                        $("#composeApiMgrDetail_bianPai_descApiButton").attr("operateorId",operateorId);
                        $("#composeApiMgrDetail_bianPai_descApiButton").trigger("click");
                    }else if(operatorType == "decide"){
                        $("#composeApiMgr_bianPai_decideButton").attr("operateorId",operateorId);
                        $("#composeApiMgr_bianPai_decideButton").trigger("click");
                        $("#composeApiMgrDetail_bianPai_decideButton").attr("operateorId",operateorId);
                        $("#composeApiMgrDetail_bianPai_decideButton").trigger("click");
                    }
                }
                return false;
            },
            ...
            onLinkDbSelect:function (linkId) {   //新增双击事件
                $("#composeApiMgr_bianPai_linkOperateButton").attr("linkId",linkId);
                $("#composeApiMgr_bianPai_linkOperateButton").trigger("click");
                $("#composeApiMgrDetail_bianPai_linkOperateButton").attr("linkId",linkId);
                $("#composeApiMgrDetail_bianPai_linkOperateButton").trigger("click");
            },
        ...
        _initEvents: function() {
            ...
            this.objs.layers.operators.on('dblclick', '.flowchart-operator', function(e) {//新增 绑定双击事件
                if ($(e.target).closest('.flowchart-operator-connector').length == 0) {
                    self.options.onOperatorDbSelect($(this).data("operator_id"));
                }
            });
            ...
            this.objs.layers.links.on('dblclick', '.flowchart-link', function(e) { //新增 连线增加双击事件
                self.options.onLinkDbSelect("create_operator_link_"+$(this).data('link_id'));
            });
        ...
        _drawLink: function(linkId) {
            ...
            group.setAttribute('id','create_operator_link_'+linkId);
            group.setAttribute('data-link_id', linkId);
            overallGroup.appendChild(group);

            var shape = document.createElementNS("http://www.w3.org/2000/svg", "path");
            //shape.setAttribute("id", "myPath"+linkId);
            ...
            /*var linkText = document.createElementNS("http://www.w3.org/2000/svg", "text");
            linkText.setAttribute("text-anchor","middle");
            linkText.setAttribute("id","myText"+linkId);
            linkText.setAttribute("dy","-10");
            group.appendChild(linkText);

            var shape = document.createElementNS("http://www.w3.org/2000/svg", "textPath");
            shape.appendChild(document.createTextNode("参数配置："+linkId));
            shape.setAttribute("xlink:href","#myPath"+linkId);
            shape.setAttribute("startOffset","50%");
            linkText.appendChild(shape);*/

            this._refreshLinkPositions(linkId);
            this.uncolorizeLink(linkId);

            //$("#myText"+linkId).html($("#myText"+linkId).html());
        },
        ...
        _getOperatorFullElement: function(operatorData,operatorType,operatorId) {
            var infos = this.getOperatorCompleteData(operatorData);

            var $operator = $('<div class="flowchart-operator" id="'+operatorId+'"></div>');
        ...
        createOperator: function(operatorId, operatorData, operatorType) {
            operatorData.internal = {};
            this._refreshInternalProperties(operatorData);

            var fullElement = this._getOperatorFullElement(operatorData,operatorType,operatorId);
