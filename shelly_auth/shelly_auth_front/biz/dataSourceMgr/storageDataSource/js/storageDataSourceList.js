define(["jquery", "common", "compont", "wandaComp", "wandaCompR","biz/dataSourceMgr/storageDataSource/js/dataSource_popup"], function ($, common, compont, wandaComp, wandaCompR,dataSource_popup) {
    var sourceType = "2";
    var dataBaseTypeDic={};//数据库类型字典
    var dataSourceStatusDic={};//数据源状态字典
    var dataSourceTypeDic={};//数据源类型字典
    var dataBaseVersionDic={};//数据库版本字典表
    var characterSetDic = {};//字符集字典表
    //列表Grid
    var serverListGrid = {
        rows: "10",
        pagerParam: {
        ...
        gridColums: [
            {
                ...
                template:function(data){
                    ...
                    if(status==1||status==3){
                        var changeStatus = 2;
                        html+="<a class='btn btn-default' style='margin-left: 10px;' onclick='showStorageDetail("+id+")'>查看</a>";
                        html+="<a class='btn btn-primary' style='margin-left: 10px;' onclick='updateStorageStatus("+id+","+changeStatus+")'>启用</a>";
                    }else if(status==2){
                        var changeStatus = 3;
                        html+="<a class='btn btn-default' style='margin-left: 10px; ' onclick='showStorageDetail("+id+")'>查看</a>";
                        html+="<a class='btn btn-danger' style='margin-left: 10px;'  onclick='updateStorageStatus("+id+","+changeStatus+")'>停用</a>";
                    }
                    return html;
                }
            }
        ...
        refreshDatas: function (index) {
            ...
            common.ajaxPost("dsDataSource/sourceList", param, successFun, null, null, $("#storageDataSourceList"));
        },
    ...
    var serverList_addBtn = {
        ...
        optionObj: {
            ...
            maxHeight: "",
            title: "新增数据源",
            content: "biz/dataSourceMgr/storageDataSource/html/dataSource_popup.html"
        },
        getInst: function () {
            ...
            $("#dataSourceList_delBtn").on("click", function () {
                ...
                if (selectDelete.length == 0) {
                    ...
                    common.jqConfirm.confirm({
                        content: "是否确认删除？",
                        call: function () {
                            common.ajaxDelete("dsDataSource/deleteDataSource", param, successFun, null, null, $("#storageDataSourceList"));
                        }
                    });
                }
            });
    ...
    var dataSourceList_updateStatus = {
        init:function(){
            $('#dataSourceList_updateStatus').on('click',function(){
                ...
                var param = {
                    dataSourceId:StoragedoId,
                    status:StoragedoStatus
                };
                var content = "是否确认修改";
                if(StoragedoStatus=="2"){
                    content = "是否确认启用该数据源？";
                }else if(StoragedoStatus=="3"){
                    content = "是否确认停用该数据源？";
                }
                common.jqConfirm.confirm({
                    content: content,
    ...
    var init = function () {
        wandaComp.elementControl($("#storageDataSourceList"));
        initDataBaseTypeDic();
...
//启用停用按钮
function updateStorageStatus(id,status){
    StoragedoId = id;
    StoragedoStatus = status;
